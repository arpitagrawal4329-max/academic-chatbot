import streamlit as st  # Fixed capitalized "Import"
...
with tab2:
    ...
    # Upgraded equations to clear LaTeX layouts inside the container styles
    if subject_choice == "Physics (Class 11/12 Core)":
        st.markdown("<div class='feature-card'><h4>🌌 Mechanics & Kinematics</h4></div>", unsafe_allow_html=True)
        ...
        st.latex(r"W = \Delta K = K_f - K_i")
        st.markdown("**Universal Gravitation:**")
        st.latex(r"F = G \frac{m_1 m_2}{r^2}")
        
    elif subject_choice == "Chemistry Physical Equations":
        st.markdown("<div class='feature-card'><h4>🧪 Thermodynamics & Gas Laws</h4></div>", unsafe_allow_html=True)
        st.markdown("**Ideal Gas Law State:**")
        st.latex(r"P \cdot V = n \cdot R \cdot T")
        st.markdown("**Gibbs Free Energy Criteria:**")
        st.latex(r"\Delta G = \Delta H - T\Delta S")
        
    elif subject_choice == "Mathematics Essentials":
        st.markdown("<div class='feature-card'><h4>📐 Calculus & Algebra Matrix</h4></div>", unsafe_allow_html=True)
        st.markdown("**Quadratic Formula Roots:**")
        st.latex(r"x = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}")
        st.markdown("**Fundamental Derivative:**")
