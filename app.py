import streamlit as st
from google import genai
from google.genai import types
import os
import uuid

# ==========================================
# 1. PAGE CONFIGURATION & APP THEMING
# ==========================================
st.set_page_config(
    page_title="Academic AI Assistant",
    page_icon="📚",
    layout="centered",
    initial_sidebar_state="expanded"
)

# Custom CSS to construct a authentic expanding pill-shaped Gemini input container
st.markdown("""
    <style>
    .main {
        background-color: #131314;
        color: #e3e3e3;
    }
    .credit-box {
        background-color: #1e1e24;
        padding: 15px;
        border-radius: 10px;
        border-left: 5px solid #4b6cb7;
        margin-bottom: 25px;
    }
    .chat-bubble-user {
        background-color: #0b554a;
        padding: 12px;
        border-radius: 10px 10px 0px 10px;
        margin: 10px 0px;
        max-width: 85%;
        margin-left: auto;
        color: white;
    }
    .chat-bubble-ai {
        background-color: #202124;
        padding: 12px;
        border-radius: 10px 10px 10px 0px;
        margin: 10px 0px;
        max-width: 85%;
        margin-right: auto;
        color: #e3e3e3;
        border: 1px solid #3c4043;
    }
    .sidebar-text {
        color: #9aa0a6;
        font-size: 14px;
        font-weight: 500;
        margin-bottom: 10px;
    }
    
    /* Unified pill-shaped Gemini pod wrapper */
    .unified-grey-pod {
        background-color: #1e1e24 !important;
        border: 1px solid #3c4043 !important;
        border-radius: 28px !important;
        padding: 8px 16px !important;
        margin-top: 10px;
    }
    
    /* Eliminate default spacing labels inside layout elements */
    div[data-testid="stTextArea"] label, div[data-testid="stFileUploader"] label {
        display: none !important;
    }
    div[data-testid="stTextArea"], div[data-testid="stFileUploader"] {
        padding: 0px !important;
        margin: 0px !important;
    }
    
    /* Make text area completely borderless and allow vertical expansions */
    div[data-testid="stTextArea"] textarea {
        background-color: transparent !important;
        border: none !important;
        color: #e3e3e3 !important;
        padding: 0px !important;
        font-size: 16px !important;
        resize: none !important;
        min-height: 38px !important;
    }
    div[data-testid="stTextArea"] textarea:focus {
        box-shadow: none !important;
    }
    
    /* Horizontal control flex alignment layout rules */
    .chat-controls-row {
        display: flex !important;
        flex-direction: row !important;
        align-items: flex-end !important;
        gap: 10px !important;
        width: 100% !important;
    }
    
    /* Gemini-style Action Buttons */
    .stButton>button {
        background-color: transparent !important;
        color: #9aa0a6 !important;
        border: none !important;
        font-size: 22px !important;
        height: 38px !important;
        width: 38px !important;
        padding: 0px !important;
        margin: 0px !important;
        display: inline-flex !important;
        align-items: center !important;
        justify-content: center !important;
    }
    
    /* Circular blue accents button configuration for the send action icon */
    button[key*="send_arrow"] {
        background-color: #1a73e8 !important;
        color: white !important;
        border-radius: 50% !important;
    }
    button[key*="send_arrow"]:hover {
        background-color: #1557b0 !important;
    }
    .stButton>button:hover {
        color: #e3e3e3;
    }
    </style>
""", unsafe_allow_html=True)

# ==========================================
# 2. BACKEND AUTHENTICATION SETUP
# ==========================================
api_key = os.environ.get("GEMINI_API_KEY")
client_ready = False

if not api_key:
    st.error("🔑 API Key Configuration Missing! Check your Streamlit Secrets drawer.")
else:
    try:
        client = genai.Client(api_key=api_key)
        client_ready = True
    except Exception as e:
        st.error(f"Engine Core Failure: {e}")

# ==========================================
# 3. SESSION STATE CONVERSATION CONFIG
# ==========================================
if "all_chats" not in st.session_state:
    st.session_state.all_chats = {}

if "active_chat_id" not in st.session_state:
    first_id = str(uuid.uuid4())
    st.session_state.all_chats[first_id] = {
        "title": "New Study Session",
        "history": []
    }
    st.session_state.active_chat_id = first_id

if "show_plus_menu" not in st.session_state:
    st.session_state.show_plus_menu = False

# ==========================================
# 4. SIDEBAR NAVIGATION MENU (RECENTS BAR)
# ==========================================
with st.sidebar:
    if st.button("➕ New Chat Session", key="new_chat_btn_sidebar"):
        new_id = str(uuid.uuid4())
        st.session_state.all_chats[new_id] = {
            "title": "New Study Session",
            "history": []
        }
        st.session_state.active_chat_id = new_id
        st.session_state.show_plus_menu = False
        st.rerun()
        
    st.markdown("---")
    st.markdown('<div class="sidebar-text">Recents</div>', unsafe_allow_html=True)
    
    for chat_id, chat_data in list(st.session_state.all_chats.items()):
        col_select, col_del = st.columns([5, 1])
        with col_select:
            truncated_title = chat_data['title'] if len(chat_data['title']) <= 25 else chat_data['title'][:22] + "..."
            btn_style = f"🔥 {truncated_title}" if chat_id == st.session_state.active_chat_id else truncated_title
            if st.button(btn_style, key=f"select_session_{chat_id}"):
                st.session_state.active_chat_id = chat_id
                st.session_state.show_plus_menu = False
                st.rerun()
        with col_del:
            if st.button("❌", key=f"delete_session_{chat_id}"):
                del st.session_state.all_chats[chat_id]
                if st.session_state.active_chat_id == chat_id:
                    remaining_ids = list(st.session_state.all_chats.keys())
                    st.session_state.active_chat_id = remaining_ids[0] if remaining_ids else None
                st.rerun()
                
    st.markdown("---")
    if st.button("🗑️ Clear Entire History", key="clear_all_global_history_btn"):
        st.session_state.all_chats = {}
        st.session_state.active_chat_id = None
        st.session_state.show_plus_menu = False
        st.rerun()

if not st.session_state.active_chat_id or st.session_state.active_chat_id not in st.session_state.all_chats:
    fallback_id = str(uuid.uuid4())
    st.session_state.all_chats[fallback_id] = {"title": "New Study Session", "history": []}
    st.session_state.active_chat_id = fallback_id

current_session = st.session_state.all_chats[st.session_state.active_chat_id]

# ==========================================
# 5. DYNAMIC CREDIT WELCOME SCREEN BANNER
# ==========================================
if not current_session["history"]:
    st.markdown("""
        <div class="credit-box">
            <h2 style='margin-top:0; color:#4b6cb7; font-size:24px;'>🎓 Academic AI Assistant</h2>
            <p style='margin-bottom:5px; font-size:14px;'><strong>Developed by:</strong> Harshit Agrawal & Umar Fahrukh</p>
            <p style='margin-bottom:5px; font-size:14px;'><strong>School:</strong> Acharya Tulsi Sarvodaya Bal Vidyalaya</p>
            <p style='margin-bottom:0; font-size:14px; color:#a0a0a0;'><strong>Guided by:</strong> Gopal Meena Sir</p>
        </div>
    """, unsafe_allow_html=True)

# ==========================================
# 6. APPLICATION NAVIGATION TABS
# ==========================================
tab1, tab2, tab3 = st.tabs([
    "🔍 Question Solver", 
    "📝 Topic Summary", 
    "📐 Chapter Formula Sheet"
])

def render_chatbot_interface(mode_name, input_placeholder):
    st.subheader(f"Session Focus: {current_session['title']}")
    
    st.write("---")
    if not current_session["history"]:
        if mode_name == "Question Solver":
            st.caption("Submit an academic homework query or problem photo below for a step-by-step breakdown solution.")
        elif mode_name == "Topic Summary":
            st.caption("Type an academic topic name below to generate custom structural summary notes.")
        else:
            st.caption("Provide a syllabus chapter title below to construct an exhaustive formula matrix sheet.")
    else:
        for chat in current_session["history"]:
            if chat["role"] == "user":
                st.markdown(f'<div class="chat-bubble-user"><b>You:</b><br>{chat["text"]}</div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="chat-bubble-ai"><b>Tutor AI:</b><br>{chat["text"]}</div>', unsafe_allow_html=True)
    st.write("---")

    # ONE SINGLE GREY CAPSULE BOX CONTAINING FILE DISPLAYS & TEXT ROWS
    st.markdown('<div class="unified-grey-pod">', unsafe_allow_html=True)
    uploaded_files = None

    if st.session_state.show_plus_menu:
        st.markdown("<p style='margin:0 0 6px 0; font-size:12px; color:#4b6cb7; font-weight:bold;'>ATTACH FILE ASSIGNMENTS (IMAGES/PDF)</p>", unsafe_allow_html=True)
        uploaded_files = st.file_uploader(
            label=f"inline_media_{mode_name}", 
            type=["png", "jpg", "jpeg", "pdf"], 
            accept_multiple_files=True, 
            key=f"inline_field_{mode_name}"
        )
        st.markdown("<hr style='margin:10px 0; border:0; border-top:1px solid #3c4043;'>", unsafe_allow_html=True)

    # Base Horizontal Control Elements Row Layout
    st.markdown('<div class="chat-controls-row">', unsafe_allow_html=True)
    col_p, col_t, col_b = st.columns([1, 14, 1])

    with col_p:
        if st.button("＋", key=f"pod_toggle_{mode_name}"):
            st.session_state.show_plus_menu = not st.session_state.show_plus_menu
            st.rerun()
            
    with col_t:
        # Replaced text_input with expanding text_area to ensure paragraph visibility
        user_query = st.text_area(
            label=f"query_txt_{mode_name}",
            placeholder=input_placeholder,
            height=38,
            key=f"message_field_{mode_name}"
        )
        
    with col_b:
        submit_btn = st.button("➔", key=f"send_arrow_{mode_name}")
        
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    # ==========================================
    # 7. BACKEND SERVICE ROUTING CORE ENGINE
    # ==========================================
    if submit_btn:
        has_text = bool(user_query.strip())
        has_files = bool(uploaded_files) if uploaded_files else False

        if not client_ready:
            st.error("AI core processing channel is offline.")
        elif not has_text and not has_files:
            st.warning("Please provide input text or choose a file first.")
        else:
            contents_payload = []
            
            global_academic_lock = (
                "ABSOLUTE RULE: You are an academic high school tutor robot. You are NOT a conversational partner, friend, or emotional advisor. "
                "If the user query is conversational chitchat, relationship problems (e.g., questions about girlfriends, friends, emotional feelings), "
                "personal life advice, sports, gaming, or anything non-academic, you must completely refuse to answer. "
                "Respond verbatim with exactly: 'I am an academic assistant designed only for high school syllabus problems. Please enter a valid question.'\n\n"
            )
            
            if mode_name == "Question Solver":
                system_scaffolding = global_academic_lock + (
                    "MODE RULE: You are operating strictly in Question Solver mode. You can ONLY solve specific science, math, or humanities "
                    "questions, calculations, and homework assignments. If the user asks for a general chapter summary or notes instead of a question, "
                    "respond verbatim with exactly: 'I can't process anything except questions.' Otherwise, provide a step-by-step breakdown solution.\n"
                )
            elif mode_name == "Topic Summary":
                system_scaffolding = global_academic_lock + (
                    "MODE RULE: You are operating strictly in Topic Summary mode. You can ONLY provide clean, structural high-yield summaries "
                    "for academic topic terms. If the user posts an equation or problem to solve step-by-step, respond verbatim with exactly: "
                    "'I can't process anything except topic summaries.' Otherwise, output a neat conceptual review sheet.\n"
                )
            else:
                system_scaffolding = global_academic_lock + (
                    "MODE RULE: You are operating strictly in Chapter Formula Sheet mode. The user will input an academic chapter name. "
                    "Extract all primary physics, math, or chemistry formulas from that chapter in clean formula blocks, and list the core "
                    "sub-topics with an exact 2-line description under each.\n"
                )
                
            history_context = system_scaffolding + "Review context window logs:\n"
            for past_msg in current_session["history"][-6:]:
                history_context += f"{past_msg['role']}: {past_msg['text']}\n"
            contents_payload.append(history_context)

            display_log_text = f"<b>[{mode_name}]</b> {user_query}" if user_query.strip() else f"<b>[{mode_name}]</b> [Sent Attachment]"
            if has_files:
                display_log_text += f" <i>(+{len(uploaded_files)} files attached)</i>"

            if has_files:
                for f in uploaded_files:
                    contents_payload.append(types.Part.from_bytes(data=f.read(), mime_type=f.type))

            if has_text:
                contents_payload.append(user_query)
            else:
                contents_payload.append("Fulfill the mode requirements on the attached parameters.")

            current_session["history"].append({"role": "user", "text": display_log_text})

            with st.spinner("Processing..."):
                try:
                    response = client.models.generate_content(
                        model="gemini-2.5-flash",
                        contents=contents_payload
                    )
                    current_session["history"].append({"role": "model", "text": response.text})
                    if len(current_session["history"]) <= 2 and has_text:
                        current_session["title"] = f"[{mode_name.split()[0]}] " + user_query[:18].strip()
                    
                    st.session_state.show_plus_menu = False
                    st.rerun()
                except Exception as api_err:
                    st.error(f"Execution Error: {api_err}")

# Connect tabs to interfaces
with tab1:
    render_chatbot_interface("Question Solver", "Ask your academic question here...")

with tab2:
    render_chatbot_interface("Topic Summary", "Enter a topic name to get summary notes...")

with tab3:
    render_chatbot_interface("Chapter Formula Sheet", "Enter a chapter name to get formulas...")
  
