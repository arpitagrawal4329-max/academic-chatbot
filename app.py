...
# Custom CSS styling injected to create a modern chat thread and compact input bar
st.markdown("""
    <style>
    .main {
    ...
    /* Style for the send arrow button */
    .stButton>button {
...
if not api_key:
    ...
    st.info("Please add your key in the Streamlit Advanced Settings -> Secrets drawer.")
else:
    try:
...
# ------------------------------------------
# TAB 1: STEP-BY-STEP SOLVER (COMPACT DYNAMIC INPUT)
# ------------------------------------------
with tab1:
    ...
    # Compact Gemini-Style Layout Bar using Columns
    ...
    with col_btn:
        # Arrow button to send inputs
        submit_btn = st.button("➔", key="send_arrow_btn")

    # Processing Input Actions (Works with Text-only, File-only, or Both!)
    if submit_btn:
        ...
        if not client_ready:
            ...
            st.warning("Please type a question or choose a file first.")
        else:
            contents_payload = []
            
            # Formulate history logs context wrapper
            ...
            # Build readable chat history tag strings
            if has_text and has_files:
                display_log_text = f"{user_query} <i>(+{len(uploaded_files)} file attachments)</i>"
            elif has_files:
                display_log_text = f"<i>[Sent {len(uploaded_files)} file attachments for analysis]</i>"
            else:
                display_log_text = user_query

            # Process any attached file binaries into standard Gemini parts payload
            if has_files:
            ...
            # Process textual query updates into model payload
            if has_text:
                contents_payload.append(user_query)
            else:
                contents_payload.append("Analyze the attached document page and provide a step-by-step breakdown explanation.")

            # Append user entry log to current state view
            st.session_state.chat_history.append({"role": "user", "text": display_log_text})
            ...
            with st.spinner("Tutor is calculating solution steps..."):
                try:
                    ...
                    st.error("The calculation core encountered an error analyzing this query payload.")
